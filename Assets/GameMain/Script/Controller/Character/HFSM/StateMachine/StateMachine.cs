...
using GameMain.Script.Controller.Character.HFSM.Exceptions;
...
namespace GameMain.Script.Controller.Character.HFSM.StateMachine
{
	/// <summary>
		...
		IActionable<TEvent>
	{
		...
		private (TStateId state, bool isPending, Action successAction) pendingState = (default, false, null);
		...
		/// </summary>
		/// <param name="context">
		/// String message for which action the fsm should be initialised for.
		/// </param>
		...
		public void StateCanExit()
		{
			if (pendingState.isPending)
			{
				...
				var action = pendingState.successAction;
				...
				pendingState = (default, false, null);
				action?.Invoke();
				ChangeState(state);
			}
		...
		private void ChangeState(TStateId name)
		{
			...
			if (!nameToStateBundle.TryGetValue(name, out var bundle) || bundle.state == null)
			{
		...
		public void RequestStateChange(TStateId name, bool forceInstantly = false, Action successAction = null)
		{
			...
			{
				pendingState = (name, true, successAction);
				activeState.OnExitRequest();
				/*
		...
		private bool TryTransition(TransitionBase<TStateId> transition)
		{
			...
			RequestStateChange(transition.to, transition.forceInstantly, transition.successAction);
		...
		public override void OnEnter()
		{
			...
			foreach (var transition in triggerTransitionsFromAny.Values
		...
		{
			return transitionsFromAny
				.Where(transition => !EqualityComparer<TStateId>.Default.Equals(transition.to, activeState.name))
				.Any(TryTransition);
		}
		...
		{
			return activeTransitions.Any(TryTransition);
		}
		...
		public override void OnLogic()
		{
			...
			var hasChangedState = TryAllGlobalTransitions();
